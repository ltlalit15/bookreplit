...
// console.log("Uploads folder path:", uploadDir); // Debugging

// // ✅ Multer Configuration for Multiple File Uploads
// const storage = multer.diskStorage({
//     destination: (req, file, cb) => {
//         cb(null, uploadDir); // ✅ Use fixed uploads directory
//     },
...
//     { name: "image", maxCount: 1 },          // Image Upload
//     { name: "audio_book", maxCount: 1 },     // Audio Book Upload
//     { name: "flip_book", maxCount: 1 }       // Flip Book Upload
// ]);
...
// Backblaze code
// export const addBook = async (req, res) => {
//     // Using upload middleware correctly
...
//                         bucketId: "your_bucket_id",  // Replace with your actual Backblaze B2 bucket ID
//                         fileName: `book_image_${Date.now()}.jpg`,
//                         data: req.files["image"][0].buffer,
//                         contentType: req.files["image"][0].mimetype
//                     });

//                     // Log the full upload response to check for fileUrl
//                     console.log('Image Upload Response:', uploadResponse);

//                     // Ensure the fileUrl is returned and not undefined
//                     if (uploadResponse?.data?.fileUrl) {
//                         imageUrl = uploadResponse.data.fileUrl;
//                     } else {
//                         throw new Error('File URL is undefined in the upload response');
//                     }
//                 } catch (error) {
//                     console.error("Image upload failed:", error);
//                     return res.status(500).json({ message: "Image upload failed", error: error.message });
...
//                 try {
//                     const uploadResponse = await b2.uploadFile({
//                         bucketId: "your_audio_bucket_id",  // Replace with your actual audio bucket ID
//                         fileName: `book_audio_${Date.now()}.mp3`,
//                         data: req.files["audio_book_url"][0].buffer,
//                         contentType: req.files["audio_book_url"][0].mimetype
//                     });
...
//                     // Ensure the fileUrl is returned and not undefined
//                     if (uploadResponse?.data?.fileUrl) {
//                         audioUrl = uploadResponse.data.fileUrl;
//                     } else {
//                         throw new Error('File URL is undefined in the upload response');
//                     }
//                 } catch (error) {
...
//             // Insert book into MySQL database
//             const [bookResult] = await pool.query(
//                 "INSERT INTO book (category_id, book_name, description, status, image, audio_book_url, flip_book_url, author) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
//                 [categoryIdNum, book_name, description, statusValue, imageUrl, audioUrl, flipBookUrl, author]
//             );
...
//             // Insert questions into MySQL if provided
//             if (questions) {
//                 const parsedQuestions = JSON.parse(questions);
//                 if (Array.isArray(parsedQuestions)) {
//                     const questionInsertPromises = parsedQuestions.map(async (question) => {
